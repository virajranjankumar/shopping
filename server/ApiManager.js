...
/*
  constructor() {
    ...
    this.apiUrlEndpoint = 'http://localhost:3000';
    this.apiCategoriesEndpoint = this.apiUrlEndpoint + '/all';
    this.apiProductEndpoint = this.apiUrlEndpoint + '/all';
    this.MAX_RELATED_PRODUCTS = 11;


    var apiUrlValues = [
      ['books-childrens', '200368502'],
      ['books-textbooks', '200368503'],
      ['furnitures-tables', '200368507'],
      ['furnitures-sofas', '200368509'],
      ['high-low', 'priceHtoL'],
      ['low-high', 'priceLtoH']
    ];
    this.apiUrlMap = new Map(apiUrlValues);
      ...
      getCategoryUrl(categoryId, sort) {
          ...
          var apiUrlParams = (sort != undefined ? '_sort=' + this.apiUrlMap.get(sort) : '');
          return this.apiCategoriesEndpoint + "/" + categoryId + '?' + apiUrlParams;
      }
      ...
      removeItem: function (productId, color, size) {
        ...
        for (var i = 0; i < this.cartItems.length; i++) {
          if (this.cartItems[i].productId === productId && 
              this.cartItems[i].color === color && 
              this.cartItems[i].size === size) {
